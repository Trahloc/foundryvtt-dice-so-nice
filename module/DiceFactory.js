...
export class DiceFactory {
	...
	constructor() {
		...
		for(let i in CONFIG.Dice.terms){
			...
			if(![Coin, FateDie, Die].includes(term)){
				...
				if([3, 4, 6, 8, 10, 12, 14, 16, 20, 24, 30].includes(objTerm.faces)){
	...
	create_d14_geometry(radius, scopedScale) {
		...
		geom.cannon_shape = this.create_geom2(vertices, faces, radius * 0.85);
		return geom;
	}
	...
	create_d16_geometry(radius, scopedScale) {
		...
		geom.cannon_shape = this.create_geom2(vertices, faces, radius * 0.83);
		return geom;
	}
	...
	create_d30_geometry(radius, scopedScale) {
		...
		geom.cannon_shape = this.create_geom2(vertices, faces, radius);
		return geom;
	}
	...
	create_shape2(vertices, faces, radius) {
		var cv = new Array(vertices.length), cf = new Array(faces.length);
		for (var i = 0; i < vertices.length; ++i) {
			var v = vertices[i];
			cv[i] = new CANNON.Vec3(v.x * radius, v.y * radius, v.z * radius);
	...
	create_geom2(vertices, faces, radius) {
		var vectors = new Array(vertices.length);
		for (var i = 0; i < vertices.length; ++i) {
			vectors[i] = (new THREE.Vector3).fromArray(vertices[i]).normalize();
		}
		let cannon_shape = this.create_shape2(vectors, faces, radius);
		return cannon_shape;
	}
}
